...
def extrair_nacionalidades(df: pd.DataFrame, nome_dataset: str) -> Optional[List[str]]:
    ...
    """
    if df is None:
        print(f"❌ Dataset {nome_dataset} não disponível")
        return None
...
def analisar_setores_atividade(df: pd.DataFrame) -> Dict[str, str]:
    ...
    if df is None:
        print("❌ Dataset de setores não disponível")
        return {}
...
class AnalisadorMicrodadosINE:
    ...
    def verificar_disponibilidade_dados(self) -> None:
        ...
        if arquivos_disponiveis == 0:
            print("⚠️  AVISO: Nenhum arquivo de dados encontrado!")
            ...
            if nome_interno in self.config.DATASETS:
                ...
                if caminho.exists():
                    ...
                    if df is not None:
                        ...
                        print(f"❌ Falha ao carregar: {nome_descritivo}")
                else:
                    print(f"❌ Arquivo não encontrado: {nome_descritivo}")
    ...
    def analisar_contexto_temporal_geografico(self) -> None:
        ...
        for nome_interno, df in self.datasets.items():
            ...
            if colunas_geograficas:
                print(f"   🗺️  Colunas geográficas: {colunas_geograficas}")
            else:
                print(f"   🗺️  Nível geográfico: Nacional (agregado)")
