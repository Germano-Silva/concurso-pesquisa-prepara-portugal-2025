...
Os dados apresentados na Tabela 13 são **ESTIMATIVAS EXPLORATÓRIAS** derivadas da correlação entre distribuição setorial e perfil educacional por nacionalidade. **NÃO CONSTITUEM DADOS PRIMÁRIOS DIRETOS** de escolaridade por setor econômico. As fontes oficiais consultadas (INE Censos 2021, AIMA) não disponibilizam cruzamento direto entre nível educacional e setor de atividade para população estrangeira. As percentagens são intervalos aproximados baseados em inferências estatísticas a partir do perfil conhecido das principais nacionalidades em cada setor (Tabela 9). Estes valores devem ser interpretados como **indicadores orientativos** para compreensão de padrões gerais, não como medições precisas. Investigações futuras com acesso a microdados censitários seriam necessárias para validação rigorosa destas estimativas.
